...
namespace tem::avx2::intern
{
    ...
    public:
        ...
        T y;
    public:
        TEM_AVX2_FMA3_SIMD constexpr vec2(const T x, const T y);
        TEM_AVX2_FMA3_SIMD constexpr vec2(const T v);

        TEM_AVX2_FMA3_SIMD constexpr vec2() = default;
        TEM_AVX2_FMA3_SIMD ~vec2() = default;

        TEM_AVX2_FMA3_SIMD TEM_NODISCARD constexpr const T operator[](const tem_ui32 Index) const;
        TEM_AVX2_FMA3_SIMD TEM_NODISCARD constexpr T& operator[](const tem_ui32 Index);

        TEM_AVX2_FMA3_SIMD TEM_NODISCARD constexpr bool operator<(const vec2<T>& a) const;
        TEM_AVX2_FMA3_SIMD TEM_NODISCARD constexpr bool operator<=(const vec2<T>& a) const;
        TEM_AVX2_FMA3_SIMD TEM_NODISCARD constexpr bool operator==(const vec2<T>& a) const;
        TEM_AVX2_FMA3_SIMD TEM_NODISCARD constexpr bool operator>=(const vec2<T>& a) const;
        TEM_AVX2_FMA3_SIMD TEM_NODISCARD constexpr bool operator>(const vec2<T>& a) const;

        TEM_AVX2_FMA3_SIMD TEM_NODISCARD constexpr vec2<T> operator+(const vec2<T>& a) const;
        TEM_AVX2_FMA3_SIMD TEM_NODISCARD constexpr vec2<T> operator+=(const vec2<T>& a) const;
        ...
        TEM_AVX2_FMA3_SIMD TEM_NODISCARD constexpr vec2<T> abs() const;
        TEM_AVX2_FMA3_SIMD TEM_NODISCARD constexpr vec2<T> normalize() const;
        TEM_AVX2_FMA3_SIMD TEM_NODISCARD constexpr T length() const;

        TEM_AVX2_FMA3_SIMD static constexpr T dot(const vec2<T>& a, const vec2<T>& b);
        TEM_AVX2_FMA3_SIMD static constexpr T distance(const vec2<T>& a, const vec2<T>& b);
    };

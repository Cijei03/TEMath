...
namespace tem::avx::intern
{
    ...
    private:
        vec2<T> Data[2];
    public:
        TEM_AVX_SIMD constexpr mat2() = default;
        TEM_AVX_SIMD constexpr mat2(const vec2<T>& a, const vec2<T>& b);

        TEM_AVX_SIMD ~mat2() = default;

        TEM_AVX_SIMD constexpr void makeUnit();

        TEM_AVX_SIMD constexpr void scale(const vec2<T>& value);
        TEM_AVX_SIMD constexpr void scale(const T value);

        TEM_AVX_SIMD constexpr void rotate(const ::tem::Radian<T> angle) requires(std::is_floating_point_v<T>);
        TEM_AVX_SIMD TEM_NODISCARD constexpr mat2<T> transpose();
        TEM_AVX_SIMD TEM_NODISCARD constexpr mat2<T> inverse() requires(std::is_signed_v<T>);

        TEM_AVX_SIMD TEM_NODISCARD constexpr T getDeterminant() const;

        TEM_AVX_SIMD TEM_NODISCARD constexpr mat2<T> operator*(const mat2<T>& a);
        TEM_AVX_SIMD TEM_NODISCARD constexpr mat2<T> operator+(const mat2<T>& a);
        TEM_AVX_SIMD TEM_NODISCARD constexpr mat2<T> operator-(const mat2<T>& a);
        TEM_AVX_SIMD TEM_NODISCARD constexpr vec2<T> operator*(const vec2<T>& a);

        TEM_AVX_SIMD TEM_NODISCARD constexpr vec2<T>& operator[](const uint32_t Index);
        TEM_AVX_SIMD TEM_NODISCARD constexpr const vec2<T>& operator[](const uint32_t Index) const;
        ...
        TEM_AVX_SIMD static constexpr mat2<T> unitMatrix();
    };

...
namespace tem::avx::intern
{
	...
	private:
		vec3<T> Data[3];
	public:
		TEM_AVX_SIMD constexpr mat3() = default;
		TEM_AVX_SIMD constexpr mat3(const vec3<T>& a, const vec3<T>& b, const vec3<T>& c);

		TEM_AVX_SIMD ~mat3() = default;

		TEM_AVX_SIMD constexpr void makeUnit();

		TEM_AVX_SIMD constexpr void scale(const vec3<T>& value);
		...
		TEM_AVX_SIMD TEM_NODISCARD constexpr mat3<T> transpose();
		TEM_AVX_SIMD TEM_NODISCARD constexpr mat3<T> inverse() requires(std::is_signed_v<T>);

		TEM_AVX_SIMD TEM_NODISCARD constexpr T getDeterminant() const;

		TEM_AVX_SIMD TEM_NODISCARD constexpr mat3<T> operator*(const mat3<T>& a) const;
		TEM_AVX_SIMD TEM_NODISCARD constexpr mat3<T> operator+(const mat3<T>& a) const;
		TEM_AVX_SIMD TEM_NODISCARD constexpr mat3<T> operator-(const mat3<T>& a) const;
		TEM_AVX_SIMD TEM_NODISCARD constexpr vec3<T> operator*(const vec3<T>& a) const;

		TEM_AVX_SIMD TEM_NODISCARD constexpr vec3<T>& operator[](const uint32_t Index);
		TEM_AVX_SIMD TEM_NODISCARD constexpr const vec3<T>& operator[](const uint32_t Index) const;
		...
		TEM_AVX_SIMD static constexpr mat3<T> unitMatrix();
	};
